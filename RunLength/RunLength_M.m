...
if doEncode                       % Encoding: [x] -> [b, n] --------------------
   ...
   if vectorized                  % Vectorized: --------------------------------
      % matlab and mex code were giving different answers as Matlab treats NaNs as being distinct, 
      % but Mex code treats all Nans as equal
       diffWithNan = @(x) diff(x)~=0 & ~(isnan(x(1:end-1)) & isnan(x(2:end)));

      d = [true; diffWithNan(x) ~= 0];   % TRUE if values change
